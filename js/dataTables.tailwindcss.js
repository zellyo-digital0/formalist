...
$.extend( true, DataTable.ext.classes, {
	...
	search: {
		input: "border placeholder-gray-500 lg:ml-2 px-3 py-2 rounded-lg border-gray-200 focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50 dark:bg-gray-800 dark:border-gray-600 dark:focus:border-green-500 dark:placeholder-gray-400"
	},
	length: {
		select: "border px-3 py-2 rounded-lg border-gray-200 focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50 dark:bg-gray-800 dark:border-gray-600 dark:focus:border-green-500"
	},
	processing: {
...
DataTable.ext.renderer.layout.tailwindcss = function ( settings, container, items ) {
	var row = $( '<div/>', {
			"class": items.full ?
				'py-2' :
				'block md:flex justify-between py-2'
		} )
		.appendTo( container );
	...
	$.each( items, function (key, val) {
		...
		if (val.table) {
			klass = 'w-full overflow-x-auto';
		}
		else if (key === 'start') {
			klass = 'hidden md:flex self-center align-center dark:text-white';
		}
		else if (key === 'end') {
